    ...
    <AppLayout>
      <div className="section-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <button className="btn" onClick={() => setDate(d => addDays(d, -1))}>← Préc.</button>
          <span className="section-title" style={{ textTransform: 'capitalize' }}>
            {labelDate(date)}
          </span>
          <button className="btn" onClick={() => setDate(d => addDays(d, 1))}>Suiv. →</button>
        </div>
        <button className="btn btn-primary" onClick={importerIreo} disabled={importing}>
          {importing ? '⏳ Import...' : '↺ Importer iRéo'}
        </button>
      </div>
      ...
      {seances.map(s => (
        <div key={s._id} className={`saisie-card ${s.statut === 'a_completer' ? 'orange' : 'vert'}`}>
          <div className="saisie-head">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <button
                ...
                onClick={() => editId === s._id ? setEditId(null) : (setEditId(s._id), setEditContenu(s.contenu || ''))}
              >
                ✏️
              </button>
            </div>
