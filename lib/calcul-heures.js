...
// Calcule les minutes de préparation implicite pour une journée
// = plage contractuelle - pause déj - séances réelles programmées
export function preparationImplicite(seances, dateStr, datesAvecSeances = new Set()) {
  if (!seances || seances.length === 0) return 0

  const seancesActives = seances.filter(s =>
    !['conge','congé','ferie','férié','recup'].includes(s.type)
  )
  if (seancesActives.length === 0) return 0
  ...
  // Plage contractuelle
  const debutEffectif = Math.min(debutContractuel, Math.min(...seancesActives.map(s => hm(s.heure_debut))))
  const finEffective  = Math.max(finContractuelle,  Math.max(...seancesActives.map(s => hm(s.heure_fin))))

  // Pause déjeuner
  const aMidi = seancesActives.some(s => hm(s.heure_debut) < hm('13:00') && hm(s.heure_fin) > hm('12:00'))
  const pauseDej = aMidi ? 0 : 60

  // Total plage
  const totalPlage = finEffective - debutEffectif - pauseDej

  // Minutes de séances programmées (hors récup)
  const minutesProgrammees = seancesActives.reduce((acc, s) => acc + s.duree_minutes, 0)

  return Math.max(0, totalPlage - minutesProgrammees)
}
