...
import SeancesJour from '@/components/dashboard/SeancesJour'
import HeuresChart from '@/components/dashboard/HeuresChart'

async function getDashboardData() {
  const today = new Date().toISOString().split('T')[0]
  const debutAnnee = '2025-09-01'

  // En prod ces fetch appellent les routes API internes
  ...
  return (
    <AppLayout>
      ...
      <div className="metric-grid">
        <MetricCard
          label="Heures totales (année)"
          value={stats?.total_heures ?? '—'}
          sub={`depuis sept. ${new Date().getFullYear() - 1}`}
        />
        <MetricCard
          label="Face-à-face / total"
          value={stats ? `${stats.ratio_faf} %` : '—'}
          sub="plafond 42 %"
          variant={ratioAlert ? 'alert' : ratioWarn ? 'warn' : 'ok'}
        />
        <MetricCard
          label="Séances saisies"
          value={stats?.seances_saisies ?? '—'}
          sub={`sur ${stats?.seances_total ?? '?'} au total`}
          variant="ok"
        />
        <MetricCard
          label="À compléter"
          value={stats?.seances_a_completer ?? '—'}
          sub="séances sans contenu"
          variant={stats?.seances_a_completer > 0 ? 'warn' : 'ok'}
        />
      </div>
